...
- (id) initWithGame:(Game*)game {
	...
	if ((self = [super init])) {
		...
		self.sprite = [CCSprite spriteWithFile:@"bloodcell.png"];
...
- (void) resetForPos:(CGPoint)pos
{
    ...
    [self createBox2DBodyWithPos:pos];
    [self updateNode];
    [self sleep];
}

- (void) reset {
    [self resetForPos:ccp(0, _game.screenH/2+_radius)];
}

- (void)createBox2DBodyWithPos:(CGPoint)p
{
    b2BodyDef bd;
...
- (void) createBox2DBody {
    [self createBox2DBodyWithPos:ccp(0, _game.screenH/2+_radius)];
}
